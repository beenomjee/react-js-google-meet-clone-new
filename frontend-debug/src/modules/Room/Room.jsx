...
const Room = () => {
    ...
    const onJoiningRoom = useCallback(async ({ roomData, mySocketId }) => {
        ...
        for (let socketId in roomData) {
            ...
            // sending offer
            console.log(`Sending offer to ${roomData[socketId].name}-${socketId}`);
    ...
    const onCandidate = useCallback(async ({ from, candidate }) => {
        const pc = connections[from];
        if (pc) {
            console.log(`Setting candidate of ${from}`);
            await pc.addIceCandidate(candidate);
        } else {
            console.log(`Save candidates for later setting of ${from}`);
    ...
    const onUserLeave = useCallback(async ({ socketId, name, isLeaveUserIsScreenSharer }) => {
        ...
        // delete connection
        if (pc) {
            console.log(`Closing connection of ${socketId}`);
            await pc.close();
        ...
        console.log(`Removing video of ${socketId}`);
    ...
    useEffect(() => {
        ...
        return () => {
            console.log('Closing connection from server');
            socket.close();
    ...
    useEffect(() => {
        ...
        const setRemainingCandidates = async () => {
            for (const candidate of unsetCandidates) {
                ...
                if (pc) {
                    console.log('Setting saved candidates for ' + candidate.socketId);
